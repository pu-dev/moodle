...
class mod_schedule_renderer extends plugin_renderer_base {
    ...
    /**
     * Returns HTML to display schedules of option
    ...
    public function display_options($options, $coursemoduleid, $vertical = false, $multiple = false) {
        ...
        $target = new moodle_url('/mod/schedule/view.php');
        ...
        $html .= html_writer::start_tag('ul', array('class' => 'schedules list-unstyled unstyled'));
        ...
        foreach ($options['options'] as $option) {
            ...
            $option->attributes->id = 'schedule_'.$schedulecount;
            $option->attributes->class = 'mx-1';

            $labeltext = $option->text;
            if (!empty($option->attributes->disabled)) {
                $labeltext .= ' ' . get_string('full', 'schedule');
                $availableoption--;
            }
        ...
        $html .= html_writer::empty_tag('input', array('type'=>'hidden', 'name'=>'action', 'value'=>'makeschedule'));
        $html .= html_writer::empty_tag('input', array('type'=>'hidden', 'name'=>'id', 'value'=>$coursemoduleid));

        if (empty($options['previewonly'])) {
            if (!empty($options['hascapability']) && ($options['hascapability'])) {
                if ($availableoption < 1) {
                    $html .= html_writer::tag('label', get_string('schedulefull', 'schedule'));
                } else {
                    $html .= html_writer::empty_tag('input', array(
                        'type' => 'submit',
                        'value' => get_string('savemyschedule', 'schedule'),
                ...
                if (!empty($options['allowupdate']) && ($options['allowupdate'])) {
                    $url = new moodle_url('view.php',
                            array('id' => $coursemoduleid, 'action' => 'delschedule', 'sesskey' => sesskey()));
                    $html .= html_writer::link($url, get_string('removemyschedule', 'schedule'), array('class' => 'ml-1'));
                }
            } else {
                $html .= html_writer::tag('label', get_string('havetologin', 'schedule'));
            }
        }
    ...
    /**
     * Returns HTML to display schedules result
     * @param object $schedules
     * @param bool $forcepublish
     * @return string
     */
    public function display_result($schedules, $forcepublish = false) {
        if (empty($forcepublish)) { //allow the publish setting to be overridden
            $forcepublish = $schedules->publish;
        }

        $displaylayout = $schedules->display;

        if ($forcepublish) {  //SCHEDULE_PUBLISH_NAMES
            return $this->display_publish_name_vertical($schedules);
        } else {
            return $this->display_publish_anonymous($schedules, $displaylayout);
        }
    }
     ...
     * @return string
     */
    public function display_publish_name_vertical($schedules) {
        global $PAGE, $OUTPUT;
        $html ='';
        $html .= html_writer::tag('h3',format_string(get_string("responses", "schedule")));

        $attributes = array('method'=>'POST');
        $attributes['action'] = new moodle_url($PAGE->url);
        $attributes['id'] = 'attemptsform';

        if ($schedules->viewresponsecapability) {
            $html .= html_writer::start_tag('form', $attributes);
            $html .= html_writer::empty_tag('input', array('type'=>'hidden', 'name'=>'id', 'value'=> $schedules->coursemoduleid));
            $html .= html_writer::empty_tag('input', array('type'=>'hidden', 'name'=>'sesskey', 'value'=> sesskey()));
        ...
        $table->summary = get_string('responsesto', 'schedule', format_string($schedules->name));
        $table->data = array();

        $count = 0;
        ksort($schedules->options);
        ...
        $accessiblecell->text = get_string('scheduleoptions', 'schedule');
        ...
        $usernumberheader->text = get_string('numberofuser', 'schedule');
        $columns['usernumber'][] = $usernumberheader;

        $optionsnames = [];
        foreach ($schedules->options as $optionid => $options) {
            $celloption = clone($celldefault);
            $cellusernumber = clone($celldefault);

            if ($schedules->showunanswered && $optionid == 0) {
                $headertitle = get_string('notanswered', 'schedule');
            } else if ($optionid > 0) {
                $headertitle = format_string($schedules->options[$optionid]->text);
            }
            $celltext = $headertitle;

            // Render select/deselect all checkbox for this option.
            if ($schedules->viewresponsecapability && $schedules->deleterepsonsecapability) {
                ...
                $selectalltext = get_string('selectalloption', 'schedule', $headertitle);
                $deselectalltext = get_string('deselectalloption', 'schedule', $headertitle);
        ...
        // This extra cell is needed in order to support accessibility for screenreader. MDL-30816
        $accessiblecell = new html_table_cell();
        $accessiblecell->text = get_string('userchoosethisoption', 'schedule');
            ...
            if ($schedules->showunanswered || $optionid > 0) {
                if (!empty($options->user)) {
                    ...
                    foreach ($options->user as $user) {
                        ...
                        $userfullname = fullname($user, $schedules->fullnamecapability);
                        $checkbox = '';
                        if ($schedules->viewresponsecapability && $schedules->deleterepsonsecapability) {
                        ...
                        $userimage = $this->output->user_picture($user, array('courseid' => $schedules->courseid, 'link' => false));
                        $profileurl = new moodle_url('/user/view.php', array('id' => $user->id, 'course' => $schedules->courseid));
        ...
        if ($schedules->viewresponsecapability && $schedules->deleterepsonsecapability) {
            ...
            foreach ($schedules->options as $optionid => $option) {
                if ($optionid > 0) {
                    $actionoptions['choose_'.$optionid] = get_string('chooseoption', 'schedule', $option->text);
                }
            }
            $selectattributes = [
                'data-action' => 'toggle',
            ...
            $selectnothing = ['' => get_string('chooseaction', 'schedule')];
            $select = new single_select($actionurl, 'action', $actionoptions, null, $selectnothing, 'attemptsform');
            $select->set_label(get_string('withselected', 'schedule'));
     ...
     * @return string
     */
    public function display_publish_anonymous_horizontal($schedules) {
        global $SCHEDULE_COLUMN_HEIGHT;
        debugging(__FUNCTION__.'() is deprecated. Please use mod_schedule_renderer::display_publish_anonymous() instead.',
                DEBUG_DEVELOPER);
        return $this->display_publish_anonymous($schedules, SCHEDULE_DISPLAY_VERTICAL);
    }
     ...
     * @return string
     */
    public function display_publish_anonymous_vertical($schedules) {
        global $SCHEDULE_COLUMN_WIDTH;
        debugging(__FUNCTION__.'() is deprecated. Please use mod_schedule_renderer::display_publish_anonymous() instead.',
                DEBUG_DEVELOPER);
        return $this->display_publish_anonymous($schedules, SCHEDULE_DISPLAY_HORIZONTAL);
    }

    /**
     * Generate the schedule result chart.
     *
     * Can be displayed either in the vertical or horizontal position.
     *
     * @param stdClass $schedules Schedules responses object.
     * @param int $displaylayout The constants DISPLAY_HORIZONTAL_LAYOUT or DISPLAY_VERTICAL_LAYOUT.
     * @return string the rendered chart.
     */
    public function display_publish_anonymous($schedules, $displaylayout) {
        global $OUTPUT;
        $count = 0;
        $data = [];
        $numberofuser = 0;
        ...
        foreach ($schedules->options as $optionid => $option) {
            ...
            if($schedules->numberofuser > 0) {
                $percentageamount = ((float)$numberofuser / (float)$schedules->numberofuser) * 100.0;
            }
        ...
        $series = new \core\chart_series(format_string(get_string("responses", "schedule")), $data['series']);
